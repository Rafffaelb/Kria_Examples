...
#define LMB_BRAM_BASE_ADDRESS XPAR_LMB_BRAM_0_BASEADDRESS
...
int main() {
    ...
    float counter = 0;
    int data_to_write[6] = {0, 0, 0, 0, 0, 0};

	// infinite loop
	while(1){

        float AccelX = counter * 0.5;
        float AccelY = counter * 0.5;
        float AccelZ = counter * 0.5;
        ...
        for (int i = 0; i < VECTOR_SIZE; i++) {
            u32 address = LMB_BRAM_BASE_ADDRESS + (i * 4); // Calculate address for each element
